...
async function getall() {
  ...
  const  [famfio, proffessii, catog, familyex] = await Promise.all([
    client.query('SELECT * FROM family_fio'),
    client.query('SELECT * FROM professii'),
    client.query('SELECT * FROM categ'),
    client.query('SELECT * FROM family_ex')
  ])
  return {
    famfio: famfio.rows,
    proffessii: proffessii.rows,
    catog: catog.rows,
    familyex: familyex.rows
  }
}
////
